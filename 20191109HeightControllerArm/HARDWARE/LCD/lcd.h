...
Author:       张振华
Date:         2019/08/30
Description:  1、LCD驱动程序头文件；
			  2、根据正点原子mini板子程序移植；
			  3、将数据线由PB改为PE； //
			  4、LCD_CS PB7   片选
			     LCD_RS PB6   命令/数据 标志（0读写命令，1读写数据）
				 LCD_RD PB8   从LCD读取数据
				 LCD_WR PB9   向LCD写入数据
				 LCD_BL PB10  背光
Others:     
...
////////////////////////////////////////////////////////////////////
//-----------------LCD端口定义---------------- 
#define	LCD_LED PBout(10) 				//LCD背光    	PB10   张振华


#define	LCD_CS_SET  GPIO_SetBits(GPIOB,GPIO_Pin_7)    //片选端口  		PB7  张振华
#define	LCD_RS_SET	GPIO_SetBits(GPIOB,GPIO_Pin_6)    //数据/命令 		PB6	   
#define	LCD_WR_SET	GPIO_SetBits(GPIOB,GPIO_Pin_9)   //写数据			PB9
#define	LCD_RD_SET	GPIO_SetBits(GPIOB,GPIO_Pin_8)   //读数据			PB8
								    
#define	LCD_CS_CLR  GPIO_ResetBits(GPIOB,GPIO_Pin_7)    //片选端口  		PB7
#define	LCD_RS_CLR	GPIO_ResetBits(GPIOB,GPIO_Pin_6)     //数据/命令		PB6	   
#define	LCD_WR_CLR	GPIO_ResetBits(GPIOB,GPIO_Pin_9)     //写数据			PB9
#define	LCD_RD_CLR	GPIO_ResetBits(GPIOB,GPIO_Pin_8)    //读数据			PB8  



#define DATAOUT(x) GPIO_Write(GPIOE,x);//张振华，这里改成PE端口
#define DATAIN   GPIO_ReadInputData(GPIOE)
