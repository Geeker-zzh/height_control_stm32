...
{  
	//保存校正结果!		   							   
    fm24cxx_write_len_byte(SAVE_ADDR_BASE,*((u32*)&tp_dev.xfac),4);   //保存x校正因素(将tp_dev.xfac的地址强制转换为u32类型,再取值) 
    fm24cxx_write_len_byte(SAVE_ADDR_BASE+4,*((u32*)&tp_dev.yfac),4);	//保存y校正因素(将tp_dev.yfac的地址强制转换为u32类型,再取值)  
    fm24cxx_write_len_byte(SAVE_ADDR_BASE+8,(u16)tp_dev.xoff,2);		//保存x偏移量	 
	fm24cxx_write_len_byte(SAVE_ADDR_BASE+10,(u16)tp_dev.yoff,2);		//保存y偏移量 
	fm24cxx_write_one_byte(SAVE_ADDR_BASE+12,tp_dev.touchtype);		//保存触屏类型 
	fm24cxx_write_one_byte(SAVE_ADDR_BASE+13,0X0A); 					//标记校准过了
}
//得到保存在EEPROM里面的校准值
//返回值：1，成功获取数据
//        0，获取失败，要重新校准
...
{					  
	u8 temp;
	temp=fm24cxx_read_one_byte(SAVE_ADDR_BASE+13);//读取标记字,看是否校准过！ 		 
	if(temp==0X0A)//触摸屏已经校准过了			   
	{    												 
		*((u32*)&tp_dev.xfac)=fm24cxx_read_len_byte(SAVE_ADDR_BASE,4);	//得到x校准参数(将tp_dev.xfac的地址强制转换为u32类型,再赋值)   
		*((u32*)&tp_dev.yfac)=fm24cxx_read_len_byte(SAVE_ADDR_BASE+4,4);	//得到y校准参数(将tp_dev.xfac的地址强制转换为u32类型,再赋值) 
		tp_dev.xoff=(short)fm24cxx_read_len_byte(SAVE_ADDR_BASE+8,2);		//得到x偏移量	
		tp_dev.yoff=(short)fm24cxx_read_len_byte(SAVE_ADDR_BASE+10,2);	//得到y偏移量	 
 		tp_dev.touchtype=fm24cxx_read_one_byte(SAVE_ADDR_BASE+12);		//读取触屏类型标记
